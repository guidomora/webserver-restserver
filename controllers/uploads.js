...
const showImg = async (req = request, res = response) => {
  ...
  if (model.img) {
    ...
    // puede ser que el imgPath no exista entonces vamos a averiguar si existe
    if (fs.existsSync(imgPath)) {
     return res.sendFile(imgPath) // para retornar la imagen tenemos que suar sendFile
    }
  }

  const noImgPath = path.join(__dirname, "../assets/no-image.jpg") // imagen que retorna cuando no hay imagen en un usuario/producto
